...
CREATE TABLE IF NOT EXISTS accounts (
    ...
    archive_salt    TEXT NOT NULL DEFAULT '',
...
struct RegisterRequest {
    ...
    email: String,
    password: String,
    /// Salt for the client's history key (generated by the client).
    #[serde(default)]
    archive_salt: String,
    registration_id: i64,
    identity_key: String,
...
async fn register(
    ...
    match existing {
        ...
        None => {
            sqlx::query(
                "INSERT INTO accounts (username, alias, email, password_hash, archive_salt,
...
async fn login(
    ...
    Ok(Json(serde_json::json!({
        ...
        "archive_salt": row.get::<String, _>(3),
    })))
}
